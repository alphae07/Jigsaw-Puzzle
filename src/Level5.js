...
const Level5 = () => {
  ...
  const shuffleArray = (array) => {
    ...
    <div>
        <div className="float">
            <h4>Image</h4>
        <img src={img5} alt="Puzzle" className="pic" />
        </div>
        <div className="puzz puzz5" 
                onMouseEnter={turn}>
            <JigsawPuzzle
                imageSrc = {img5}
                rows={3}
          ...
          <Link to='/level4'><button onClick={() => handleLevelChange(level - 1)}>Previous Level</button></Link>
          ...
          <Link to='/level6'><button onClick={() => handleLevelChange(level + 1)}>Next Level</button></Link>
