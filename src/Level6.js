...
const Level6 = () => {
  ...
  const shuffleArray = (array) => {
    ...
    <div>
        <div className="float">
            <h4>Image</h4>
        <img src={img6} alt="Puzzle" className="pic" />
        </div>
        <div className="puzz puzz6" 
                onMouseEnter={turn}>
            <JigsawPuzzle
                imageSrc = {img6}
                rows={3}
          ...
          <Link to='/level5'><button onClick={() => handleLevelChange(level - 1)}>Previous Level</button></Link>
          ...
          <Link to='/level7'><button onClick={() => handleLevelChange(level + 1)}>Next Level</button></Link>
